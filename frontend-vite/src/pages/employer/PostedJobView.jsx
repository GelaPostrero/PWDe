...
const PostedJobView = () => {
  ...
  const fetchApplications = async (page = 1) => {
    setLoading(true);
    try {
      const { data } = await api.get(`/api/applications/job/${jobId}/applications?page=${page}&limit=10`)
      if(data.success) {
        // Mock data for now
              ...
              {activeTab === 'applications' && (
                <div className="mt-6">
                  ...
                  <div className="flex space-x-6 border-b border-gray-200 mb-6">
                    ...
                    >
                      Proposals {jobData._count.applications}
                    </button>
                    <button 
                              ...
                              <div className="flex items-start space-x-4">
                                <div className="flex flex-col items-center">
                                  <div className={`w-16 h-16 bg-gradient-to-br ${gradientColor} rounded-full flex items-center justify-center`}>
                                    <img src={applicant.profile_picture} alt="Applicant's Profile Picture" />
